...
import { loadProfileState, resetProfileState, type ProfileState } from "@/lib/storage";
...
export default function ParentsPage() {
  ...
  function handleReset(profileId: string, name: string) {
    if (!window.confirm(t("resetConfirm", { name }))) return;
    resetProfileState(profileId);
    setStates((prev) => (prev ? { ...prev, [profileId]: loadProfileState(profileId) } : prev));
  }
            ...
            return (
              <div key={profile.id} className="notecard bg-card p-5">
                ...
                <Link
                  ...
                  className="ink-button block text-center text-sm font-bold text-white py-2 rounded-xl mb-3"
                  style={{
                ...
                {hasStarted && (
                  <button
                    type="button"
                    onClick={() => handleReset(profile.id, profile.name)}
                    className="w-full text-center text-xs font-semibold text-foreground/50 hover:text-coral-dark underline cursor-pointer"
                  >
                    {t("reset")}
                  </button>
                )}
